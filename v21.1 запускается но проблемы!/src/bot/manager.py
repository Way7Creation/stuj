    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            status = {
                ...
                "mode": "paper" if hasattr(self, 'config') and getattr(self.config, 'PAPER_TRADING', True) else "live",
                
                # Торговые пары
                "trading_pairs": {
                    "total_pairs": len(self.trading_pairs) if hasattr(self, 'trading_pairs') else 0,
                    "active_pairs": len(self.trading_pairs) if self.is_running and hasattr(self, 'trading_pairs') else 0,
                    "inactive_pairs": 0,
                    "blacklisted_pairs": 0,
                    "watchlist_pairs": 0,
                    "trending_pairs": self.trading_pairs[:5] if hasattr(self, 'trading_pairs') and self.trading_pairs else [],
                    "high_volume_pairs": self.trading_pairs[:5] if hasattr(self, 'trading_pairs') and self.trading_pairs else []
                },
                
                # Торговля
                "trading": {
                    "open_positions": len(self.positions) if hasattr(self, 'positions') else 0,
                    "pending_orders": 0,
                    "trades_today": self._get_trades_today_count() if hasattr(self, '_get_trades_today_count') else 0,
                    "daily_profit": 0.0,
                    "weekly_profit": 0.0,
                    "monthly_profit": 0.0,
                ...
                # Стратегии
                "strategies": {
                    "available_strategies": ["multi_indicator", "momentum", "mean_reversion", "breakout", "scalping", "swing"],
                    "active_strategies": [self.strategy] if hasattr(self, 'strategy') and self.is_running else [],
                    "best_performing_strategy": self.strategy if hasattr(self, 'strategy') else None,
                    "strategy_performance": {}
                },
                
                # Состояние рынка
                "market_state": {
                    "overall_trend": "BULLISH" if self.is_running else "UNKNOWN",
                    "volatility": "MEDIUM",
                    "fear_greed_index": 50,
                    "market_regime": "TRENDING" if self.is_running else "SIDEWAYS_MARKET",
                    "risk_level": "medium",
                    "btc_dominance": 0.0,
                    "eth_dominance": 0.0,
