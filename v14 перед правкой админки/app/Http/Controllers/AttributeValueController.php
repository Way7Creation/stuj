    ...
    public function index(Attribute $attribute): JsonResponse
    {
        try {
            $values = $attribute->values()
                ...
                ->get()
                ->map(function ($value) {
                    // ИСПРАВЛЕНИЕ: Подсчет через прямой запрос
                    $productsCount = \DB::table('product_attributes')
            ...
            Log::error('Ошибка получения значений атрибута: ' . $e->getMessage());
            return response()->json([
                'error' => 'Ошибка получения значений атрибута: ' . $e->getMessage()
            ], 500);
        }
    }
